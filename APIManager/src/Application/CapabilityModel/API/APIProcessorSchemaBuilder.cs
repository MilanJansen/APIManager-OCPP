    ...
    internal partial class APIProcessor : CapabilityProcessor
    {
        ...
        private bool BuildSchema(OperationCapability operation)
        {
                ...
                if (child is MessageCapabilityImp)
                {
                    ...
                    if (result = schemaProcessor.ProcessCapability(childMsg, ProcessingStage.PreProcess))
                    {
                        ...
                        if (result = schemaProcessor.ProcessCapability(childMsg, ProcessingStage.Process))
                        {
                            ...
                            this._panel.WriteInfo(this._panelIndex + 2, "Saving message...");
                            result = schemaProcessor.SaveProcessedCapability();
                            ClassCacheSlt.GetClassCacheSlt().Flush();           // Make sure to forget all types collected for previous message!
                        }
                    }
                }
            }
            ...
            if (result)
            {
                ...
                // Finally, register the operation in the processed operation list.
                ...
                this._operationContextList.Add(processingContext);
            }
            return result;
        }
