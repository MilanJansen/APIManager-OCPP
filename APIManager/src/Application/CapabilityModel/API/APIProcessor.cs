            ...
            try
            {
                switch (stage)
                {
                    ...
                    case ProcessingStage.PreProcess:
                        ...
                        {
                            // The bar-size is based on 10 steps per child. This is a wild guess but we don't want to spend hours figuring out the exact scale.
                            this._panel = ProgressPanelSlt.GetProgressPanelSlt();
                            this._panel.ShowPanel("Processing Message: " + capability.Name, capability.SelectedChildrenCount * 10);
