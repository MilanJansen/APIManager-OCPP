    ...
    internal partial class SchemaProcessor: CapabilityProcessor
    {
            ...
            try
            {
                foreach (MEAssociation assoc in node.AssociationList)
                {
                        ...
                        assoc.TypeOfAssociation == MEAssociation.AssociationType.Association)
                    {
                        ...
                        // OCPP wants all fieldnames to be camelCase...
                        associationName = TransformToCamelCase(associationName);
                        ...
                        else
                        {
                            ...
                            if (associationName == string.Empty)
                            {
                                ...
                                // OCPP wants all fieldnames to be camelCase...
                                associationName = Conversions.ToCamelCase(associationName);
                            }
        ...
        private List<SchemaAttribute> ProcessAttributes(MEClass node, bool inCommonSchema, int sequenceBase, string unqualifiedNodeName, ClassifierContext.DocScopeCode nodeDocScope)
        {
            ...
            try
            {
                foreach (MEAttribute attribute in node.Attributes)
                {
                    ...
                    // OCPP Does not like 'CodeType' so we change this to 'EnumType'...
                    classifier.Name = classifier.Name.Replace("CodeType", "EnumType");
                    ...
                    // OCPP wants all fieldnames to be camelCase and we want to replace some names...
                    attributeName = TransformToCamelCase(attributeName);
                    ...
                    if (attribute.Type == ModelElementType.Supplementary)
                    {
                        ...
                        if (classifierCtx.ContentType == ClassifierContext.ContentTypeCode.Simple || classifierCtx.ContentType == ClassifierContext.ContentTypeCode.Enum)
                        {
                            ...
                            if (this._useDocContext) classifierDocCtx.AddAttribute(attributeName, AttributeType.Supplementary, classifierCtx, targetNSToken, cardinality, attribute.Annotation);
                        }
                        else
                        {
                    ...
                    else
                    {
                        ...
                        if (this._useDocContext)
                        {
                            ...
                            attribDocCtx.AddAttribute(attributeName, AttributeType.Attribute, classifierCtx, classifierCtxID, cardinality, attribute.Annotation);
                        }
                    }
        ...
        private ClassifierContext ProcessClassifier(MEDataType classifier)
        {
            ...
            if (classifier.HasStereotype(ContextSlt.GetContextSlt().GetConfigProperty(_PrimitiveDataTypeStereotype)))
            {
                ...
                return new ClassifierContext(ClassifierContext.ContentTypeCode.Simple, classifier.Name, classifier.Name, primScope);
            }
        ...
        /// <summary>
        /// Transform a given input string to camelCase output. If the input is all upper case, the output is all lower case (e.g. "EVSE" --> "evse").
        /// If the input starts with an upper case word, it is replaced by a lowercase word (e.g. "EVSEPort" --> "evsePort").
        /// If the input is "MRID", the output is "id".
        /// </summary>
        /// <param name="name"></param>
        /// <returns></returns>
        private string TransformToCamelCase(string name)
        {
            Logger.WriteInfo("Plugin.Application.CapabilityModel.SchemaGeneration.SchemaProcessor.TransformToCamelCase >> Transforming '" + name + "'...");
            if (name == "MRID") name = "id";
            else
            {
                ...
                if (hasLowercase)
                {
                    // If lowercaseIdx == 0, the string is already in the correct format!
                    if (lowercaseIdx == 1) name = Conversions.ToCamelCase(name);
                    else if (lowercaseIdx > 1)
                    {
                        string leader = name.Substring(0, lowercaseIdx - 1);
                        leader = leader.ToLower();
                        name = leader + name.Substring(lowercaseIdx - 1);
                    }
                }
                else name = name.ToLower();
            }
            Logger.WriteInfo("Plugin.Application.CapabilityModel.SchemaGeneration.SchemaProcessor.TransformToCamelCase >> Transformed into '" + name + "'.");
            return name;
        }
    }
}
