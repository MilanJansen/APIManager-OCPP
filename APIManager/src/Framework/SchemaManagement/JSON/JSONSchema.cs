...
namespace Framework.Util.SchemaManagement.JSON
{
	internal class JSONSchema: Schema
	{
		...
		// The eventual (root) schema.
		private JSchema _skeletonSchema;
		// Contains the 'skeleton' schema as created by 'Initialize', to be completed by invoking 'Build'.
		private Uri _id;
		// The Schema ID.
		private List<JSONSchema> _externalSchemas;
		// Used to store pointers to external schemas that are referenced by this particular schema.
		private SortedList<string, JSONClassifier> _classifiers;
		// Contains all the classifiers defined in this schema.
		private SortedList<SortableSchemaElement, JSchema> _classes;
		// Contains all referenced class (ABIE) definitions in the schema.
		private SortedList<string, JSONPrimitiveType> _primitiveTypes;
		// These are all primitive types defined for this schema.
		private SortedList<string, JSchema> _rootClasses;
		// These are all root class properties, added through 'addElement'.
		...
		internal override bool AddABIEType(string className, List<MEDocumentation> annotation, List<SchemaAttribute> attributeList, List<SchemaAssociation> associationList)
		{
			...
			// [CustomData] Set AllowAdditionalProperties to true for CustomData element
			if (className.Equals("CustomData")) {
				newType.AllowAdditionalProperties = true;
				Logger.WriteInfo("Framework.Util.SchemaManagement.JSON.JSONSchema.addABIEType >> Set AdditionalProperties for " + className + " to " + (newType.AllowAdditionalProperties ? "true" : "false"));
			}
			...
			if (annotation.Count > 0) {
				...
				foreach (MEDocumentation docNode in annotation)
					description += docNode.BodyText + Environment.NewLine;
			...
			foreach (SchemaAttribute att in attributeList) {
				if (att.IsValid && att is JSONContentAttribute) {
					...
					if (att.IsMandatory)
						mandatoryProperties.Add(att.Name);
			...
			foreach (JSONAssociation associatedClass in associationList) {
				if (associatedClass.IsValid) {
					if (associatedClass.IsMandatory)
						mandatoryProperties.Add(associatedClass.Name);
			...
			if (choiceList.Count > 0) {
				if (choiceList.Values[0].IsValid) {
					...
					if (choiceList.Values[0].IsMandatory)
						mandatoryProperties.Add(choiceList.Values[0].Name);
			...
			foreach (string mandatoryProperty in mandatoryProperties)
				newType.Required.Add(mandatoryProperty);
			return result;
		}
		...
		internal override bool AddElement(string elementName, string classifierNamespace, string classifierName, List<MEDocumentation> annotation)
		{
			...
			if (this.Type == SchemaType.Message || this.Type == SchemaType.Operation) {
				...
				if (classifier != null && !this._rootClasses.ContainsKey(elementName)) {
					...
					this._rootClassifierName = classifierName;
					result = true;
				} else
					Logger.WriteError("Framework.Util.SchemaManagement.JSON.JSONSchema.addElement >> Failed to add root class '" +
					elementName + ":" + classifierName + "', classifier not found or duplicate element!");
			} else
				Logger.WriteError("Framework.Util.SchemaManagement.JSON.JSONSchema.addElement >> Schema type unsuitable for adding root classes!");
			return result;
		}
		...
		{
			if (this._primitiveTypes.ContainsKey(thisType.TypeName))
				return false;
		...
		internal override void AddSchemaReference(Schema referencedSchema)
		{
			...
			if (referencedSchema != null && referencedSchema is JSONSchema && ((JSONSchema)referencedSchema)._id != this._id) {
				...
				foreach (JSONSchema extSchema in this._externalSchemas)
					if (extSchema._id == ((JSONSchema)referencedSchema)._id)
						return;
		...
		internal void Build()
		{
			...
			try {
				// Changed for OCPP: Ignore all settings and create an 'operation' schema based on the RootClass schema element.
				this._schema = this._rootClasses.Values[0];
				this._schema.SchemaVersion = this._skeletonSchema.SchemaVersion;
				this._schema.Id = this._skeletonSchema.Id;
				this._schema.AllowAdditionalProperties = false;
				if (this._skeletonSchema.Type != JSchemaType.Null)
					this._schema.Type = this._skeletonSchema.Type;

				// Remove the original class definition from the 'classes' list...
				SortableSchemaElement rootClassifKey = new SortableSchemaElement(this._rootClassifierName, SortableSchemaElement.SortingContext.Class, 0);
				this._classes.Remove(rootClassifKey);
		...
		internal override void Merge(Schema other)
		{
			if (other != null && other is JSONSchema) {
				...
				foreach (string primType in jsonSchema._primitiveTypes.Keys) {
					...
					if (!this._primitiveTypes.ContainsKey(primType))
						this._primitiveTypes.Add(primType, jsonSchema._primitiveTypes[primType]);
				}
				...
				foreach (string classifier in jsonSchema._classifiers.Keys) {
					...
					if (!this._classifiers.ContainsKey(classifier))
						this._classifiers.Add(classifier, jsonSchema._classifiers[classifier]);
				}
				...
				foreach (SortableSchemaElement key in jsonSchema._classes.Keys) {
					...
					if (!this._classes.ContainsKey(key))
						this._classes.Add(key, jsonSchema._classes[key]);
				}
				...
				foreach (string rootClass in jsonSchema._rootClasses.Keys) {
					...
					if (!this._rootClasses.ContainsKey(rootClass))
						this._rootClasses.Add(rootClass, jsonSchema._rootClasses[rootClass]);
				}
				...
				foreach (JSONSchema extSchema in jsonSchema._externalSchemas) {
					...
					if (unique)
						this._externalSchemas.Add(extSchema);
				}
			} else
				Logger.WriteWarning("Framework.Util.SchemaManagement.JSON.JSONSchema.Merge >> Attempt to merge empty or incompatible schema!");
		}
